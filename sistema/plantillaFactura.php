<?php
	...
	class facturaPDF extends FPDF {
		function Header(){
			...
			$dniCliente = $_REQUEST['cl'];
            $noFactura = $_REQUEST['f'];
			$queryventa = mysqli_query($conexionDB,"SELECT v.IdVenta, DATE_FORMAT(v.Fecha, '%d/%m/%Y') as fecha, 
													DATE_FORMAT(v.Fecha,'%H:%i') as  hora, v.Cod_Caja,
													v.dniCliente, c.Cod_Empleado, e.Nombre as empleado,
													s.Dni, s.Nombre
													FROM ventas v 
													INNER JOIN clientes s ON v.dniCliente = s.Dni
													INNER JOIN caja c ON v.Cod_Caja = c.IdCaja
													INNER JOIN empleados e ON c.Cod_Empleado = e.IdEmpleado
													WHERE v.IdVenta = $noFactura AND v.dniCliente = $dniCliente");
			...
			$this->Image('../../img/logo_ferreteria.png',15,20,20);
			$this->Ln(10);
			$this->Cell(140);
			$this->SetFont('Arial','B',13);
			$this->Cell(26,7,mb_convert_encoding('Ticket Nº:', 'ISO-8859-1', 'UTF-8'),0,0);
			$this->SetFont('Arial','B',13);
