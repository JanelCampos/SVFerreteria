<?php
    ...
    $consulta .= " ORDER BY IdProveedor DESC";
        ...
        else
        {
            if (isset($_GET['nExcel'])) 
            {
                ...
                if ($row > 0) {
                    ...
                    echo "<th style='width:100px; border: 1px solid black;'>IdProveedor</th>";
                    echo "<th style='width:300px; border: 1px solid black;'>Nombre</th>";
                    echo "<th style='width:200px; border: 1px solid black;'>Direccion</th>";
                    echo "<th style='width:150px; border: 1px solid black;'>Telefono</th>";
                    echo "<th style='width:200px; border: 1px solid black;'>Email</th>";
                    echo "</tr>";

                    while ($fila = $result->fetch_assoc()) {
                        echo "<tr style='text-align: center;'>";
                        echo "<td style='width:100px; border: 1px solid black;'>{$fila['IdProveedor']}</td>";
                        echo "<td style='width:300px; border: 1px solid black;'>" . mb_convert_encoding($fila['Nombre'], 'ISO-8859-1', 'UTF-8') . "</td>";
                        echo "<td style='width:200px; border: 1px solid black;'>" . mb_convert_encoding($fila['Direccion'], 'ISO-8859-1', 'UTF-8') . "</td>";
                        echo "<td style='width:150px; border: 1px solid black;'>{$fila['Telefono']}</td>";
                        echo "<td style='width:200px; border: 1px solid black;'>{$fila['Email']}</td>";
                        echo "</tr>";
                    }
